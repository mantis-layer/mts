...
approved_at: 2026-08-06T11:30:00+08:00
---
...
>
> **版本演进**：
> - v1（2026-08-05）：通用 Agent Runtime 基线，三垂直切片已实现验收。
> - v2（2026-08-06）：立意升级为"数字伙伴 Runtime"，引入 Persona/Memory/ContextBuilder/Embedding；FR-014 原为"代码全量重命名 agent-* → mantis-*"。
> - v3（2026-08-06）：**重新界定 FR-014**——Go 代码（模块名 `agent-*`、package、import、CLI `mts`、env var `MTS_*`）保持稳定，**只有品牌层（README/VitePress/package.json 标题）升级为 Mantis Forge**。功能需求（FR-010/011/012/013）不变，新组件在现有 `agentcontract`/`agentcore` 等包名下实现。技术设计以架构文档与 Spike 为准。
...
v2.0 核心命题：**三个示例 Agent（Tool Loop / Research / Workflow）共享 Persona + Memory 抽象，跨会话保持身份与记忆连续性，且无需修改 agent-core。**
...
- **G7 品牌层升级（v2.0 新增，v3 重新界定）**：README/VitePress/package.json 等品牌文档升级为 Mantis Forge；**Go 代码命名（模块 `agent-*`、package、import、CLI `mts`、env var `MTS_*`）保持稳定**，不重命名。
...
- **FR-010 Persona（身份）**【v2.0 新增】：`agentcontract.Persona{ID,Name,Role,SystemPrompt,CreatedAt,UpdatedAt}`；Task 新增 PersonaID；Runtime.SubmitTask 加载关联 Persona；Storage 新增 SavePersona/GetPersona/ListPersonas；Persona 持久化跨会话复用。
- **FR-011 Memory（五层记忆）**【v2.0 新增】：`agentcontract` 定义 MemoryStore 接口（Save/Query/Delete by persona+layer）、Memory 类型、MemoryLayer enum（Working/ShortTerm/LongTerm/Preference/Skill）；`agent-runtime` 提供默认 VectorMemoryStore 实现（SQLite + sqlite-vec 纯 Go 向量 + 余弦相似度 Top-K）；MemoryStore 为接口，可被插件替换。
- **FR-012 Context Builder（注意力/工作记忆）**【v2.0 新增】：`agentcontract.ContextBuilder` 接口（输入 Persona + MemoryStore + 当前消息，返回注入相关记忆后的消息视图）；`agent-core.Options` 新增 ContextBuilder 字段，在 Steering 之后、模型调用之前执行；原 ContextHook 保留为轻量钩子（向后兼容）。
- **FR-013 EmbeddingProvider**【v2.0 新增】：`agent-model.EmbeddingProvider` 接口（`Embed(ctx, []string) ([][]float32, error)`），与 Model 并列；OpenAI adapter 同时实现两者。
- **FR-014 品牌层升级（v3 重新界定）**【v2.0 新增】：README `<h1>`、VitePress `title`/`siteTitle`、`package.json` name、CLAUDE.md 标题等品牌字符串升级为 Mantis Forge；`docs/` 文档站叙事对齐"数字伙伴"定位。**Go 代码命名保持稳定**：模块名 `agent-*`、package `agentmodel`/`agentcore` 等、import path `github.com/mantis-layer/mts/agent-*`、CLI 二进制 `mts`、env var `MTS_*` 全部不改。新组件（Persona/Memory/ContextBuilder）在现有 `agentcontract`/`agentcore` 包名下实现。
...
- **【v2.0】依赖方向**：`agent-core → agent-contract → agent-model`（包名保持 v0.1）；Persona/MemoryStore/ContextBuilder 接口上移到 agent-contract，使 core 能在不依赖 runtime 的前提下感知它们。
...
- **S11**: 三示例共享 Persona + Memory 抽象，无需修改 agent-core 即可组合（验证通用性）。
...
- **S16**: `check-deps.sh` 依赖方向无回归（代码命名不变，此条为基线回归保护）。
...
- **R4 品牌与代码命名不一致**（v3 重新界定）：产品品牌为 Mantis Forge，但 Go 代码模块名仍为 `agent-*`、CLI 为 `mts`。缓解——文档站（VitePress）与 README 明确说明"代码命名（agent-*）为内部稳定标识，产品品牌为 Mantis Forge"，类比 Kubernetes（repo `kubernetes` vs 产品 Kubernetes）。此为刻意决策，非缺陷。
...
- Status: Approved
- Approved version: 3
- Approved at: 2026-08-06T11:30:00+08:00
- v3 批准依据：重新界定 FR-014——Go 代码命名（模块 `agent-*`/package/import/CLI `mts`/env var `MTS_*`）保持稳定，只有品牌文档层升级为 Mantis Forge。功能需求（FR-010 Persona / FR-011 Memory / FR-012 ContextBuilder / FR-013 EmbeddingProvider）与 v2 一致，新组件在现有 `agentcontract`/`agentcore` 包名下实现。DAG 重构：D1（品牌文档）从地基降级为并行/最后交付，D2/D3 直接 READY。
- v2 批准依据（2026-08-06T10:30:00+08:00）：Persona 为一等实体、五层记忆全做、内置向量默认实现 + 插件可换、ContextBuilder 进 core.Options、EmbeddingProvider 独立接口、五层 Memory 与 Persona 接口上移 contract 保持依赖方向自洽。
- v1 基线（2026-08-05）：通用 Agent Runtime，三垂直切片已实现验收。

## Replanning log

- v3（2026-08-06）：FR-014 重新界定——代码保持 `agent-*`，只有品牌层升级 Mantis Forge。原因：执行 D1 时决策者重新评估，选择代码命名稳定、品牌文档层分离的策略（类似 Kubernetes 代码 repo 与产品名分离）。影响：DAG revision 1→2，D1 降级为并行节点，D2/D3 解除对 D1 的依赖成为 READY。
- v2（2026-08-06）：立意升级为数字伙伴 Runtime，引入 Persona/Memory/ContextBuilder/Embedding。
- v1（2026-08-05）：通用 Agent Runtime 基线。
